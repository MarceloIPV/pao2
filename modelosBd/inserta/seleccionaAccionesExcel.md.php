<?php
	...
	function getObtener__codigoItem($parametro1){
		...
		if(strpos($parametro1,'servicios de au')!==false){
		...
		}else if(strpos($parametro1,'culturales y s')!==false){
			...
			if (empty($sheet->getCell("A".$row)->getValue())) {
				...
				if ($sheet->getCell("A".$row)->getValue()) {
					...
					if (strpos($parametro1Act,'administrativa')!==false) {
						...
						$valorActividad=3;
					}else if(strpos($parametro1Act,'deportiva')!==false) {
						$valorActividad=4;
			...
			if ($sheet->getCell("A".$row)->getValue()=="") {
				...
				if ($sheet->getCell("D".$row)->getValue()) {
					...
					if (strpos($parametro2,'multi')!==false || $sheet->getCell("D".$row)->getValue()=="MULTIDEPORTIVOS" || $sheet->getCell("D".$row)->getValue()=="MULTI DEPORTIVOS") {
						$deporte=149;
					}else if (strpos($parametro2,'jedredrez')!==false) {
						$deporte=25;
				...
				if ($sheet->getCell("E".$row)->getValue()) {
					...
					if (strpos($parametroConversion,'esme')!==false) {
						$codigoProvincia=1;
			...
			if (4>5) {
				...
				if ($sheet->getCell("A".$row)->getValue()) {
					...
					if (strpos($sheet->getCell("A".$row)->getValue(),'ADMINISTRATIVA')!==false) {
						...
						$valorActividad=4;
					}else if(strpos($sheet->getCell("A".$row)->getValue(),'EVENTOS DE PREPARACION Y COMPETENCIA')!==false || strpos($sheet->getCell("A".$row)->getValue(),'N Y COMPETENCIA')!==false) {
						$valorActividad=5;
